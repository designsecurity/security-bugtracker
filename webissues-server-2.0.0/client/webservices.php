...
class webservice_server extends System_Api_Base
{
	function authws()
	{
    ...
    $formatterManager = new System_Api_Formatter();
		...
		try 
		{
				...
				if($same_name || $same_cve)
				{
					...
					else
					{
						...
						if($attribute_target != null)
						{
							...
							$value = $formatterManager->convertAttributeValue( $attribute_target[ 'attr_def' ], $attribute_target[ 'attr_value' ] );
							$issueManager->setValue( $issueduplicate, $attribute_target, $value );
							
							$description = $issueManager->getDescription($issueduplicate);
              $issueManager->editDescription($issueduplicate, $description["descr_text"]."\n\n".$req["description"], System_Const::TextWithMarkup);
						}
					}
			...
			{
				$value = $formatterManager->convertAttributeValue( $attribute[ 'attr_def' ], $req[$name_ws[$attribute["attr_id"]]] );
	...
	function addproject($req){
		...
		if(!Common_SecurityPlugin::valid_name($req["name"], 40))
			throw new SoapFault("Server", $GLOBALS['NAME_FILTER_INVALID']);
