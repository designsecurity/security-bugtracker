...
class WebserviceServer extends System_Api_Base
{
    ...
    private function adduser($req)
    {
        ...
        if (!Common_SecurityPlugin::validName($req["login"])) {
            throw new SoapFault("Server", $GLOBALS['NAME_FILTER_INVALID']);
        }

        if (!Common_SecurityPlugin::validName($req["username"])) {
            throw new SoapFault("Server", $GLOBALS['NAME_FILTER_INVALID']);
        }

        if (!Common_SecurityPlugin::validName($req["password"])) {
            throw new SoapFault("Server", $GLOBALS['NAME_FILTER_INVALID']);
        }
    ...
    private function getparamsfromalertid($req)
    {
        $req = (array) $req;
        ...
        if (!Common_SecurityPlugin::validId($req["id_alert"])) {
    ...
    private function finishscan($req)
    {
        ...
        if (!Common_SecurityPlugin::validId($req["id_scan"])) {
            throw new SoapFault("Server", $GLOBALS['ID_FILTER_INVALID']);
        }
    ...
    private function addscan($req)
    {
        ...
        if (!Common_SecurityPlugin::validTool($req["tool"])) {
            throw new SoapFault("Server", $GLOBALS['UNKNOWN_TOOL']);
        }

        if (!Common_SecurityPlugin::validSeverity($req["filter"])) {
            throw new SoapFault("Server", $GLOBALS['UNKNOWN_SEVERITY']);
        }

        if (!Common_SecurityPlugin::validId($req["id_folder_scans"])) {
            throw new SoapFault("Server", $GLOBALS['ID_FILTER_INVALID']);
        }
        ...
        $duplicate = false;
        $issues = Common_SecurityPlugin::getIssues($folder);
        ...
        switch ($req["tool"]) {
            case "openvas":
                $targets = Common_SecurityPlugin::findTargets($req, "dynamic");
                $issueId = Common_SecurityPlugin::runOpenvas($req, $targets);
                break;
            case "dependency-check":
                $targets = Common_SecurityPlugin::findTargets($req, "static");
                $issueId = Common_SecurityPlugin::runDependencycheck($req, $targets);
                break;
            case "arachni":
                $targets = Common_SecurityPlugin::findTargets($req, "web");
                $issueId = Common_SecurityPlugin::runArachni($req, $targets);
                break;
            case "sslscan":
                $targets = Common_SecurityPlugin::findTargets($req, "web");
                $issueId = Common_SecurityPlugin::runSslscan($req, $targets);
                break;
            case "zap":
                $targets = Common_SecurityPlugin::findTargets($req, "web");
                $issueId = Common_SecurityPlugin::runZap($req, $targets);
                break;
            case "openscat":
                $targets = Common_SecurityPlugin::findTargets($req, "static");
            ...
            case "sonar":
                $targets = Common_SecurityPlugin::findTargets($req, "static");
                $issueId = Common_SecurityPlugin::runSonar($req, $targets);
                break;
        }
    ...
    private function deletescan($req)
    {
        $req = (array) $req;
    ...
    private function deleteserver($req)
    {
        $req = (array) $req;
    ...
    private function deletecode($req)
    {
        $req = (array) $req;
    ...
    private function deleteurl($req)
    {
    ...
    private function deleteissue($req)
    {
        ...
        if (!Common_SecurityPlugin::validId($req["id_issue"])) {
            throw new SoapFault("Server", $GLOBALS['ID_FILTER_INVALID']);
        }
    ...
    private function addurl($req)
    {
        ...
        if (!Common_SecurityPlugin::validId($req["id_folder_web"])) {
            throw new SoapFault("Server", $GLOBALS['ID_FILTER_INVALID']);
        }

        if (!Common_SecurityPlugin::validName($req["name"])) {
            throw new SoapFault("Server", $GLOBALS['NAME_FILTER_INVALID']);
        }

        if (!Common_SecurityPlugin::validUrl($req["url"])) {
    ...
    private function editurl($req)
    {
        ...
        if (!Common_SecurityPlugin::validId($req["id_url"])) {
            throw new SoapFault("Server", $GLOBALS['ID_FILTER_INVALID']);
        }
    ...
    private function geturls($req)
    {
        ...
        try {
            $folder = $projectManager->getFolder($req["id_folder_web"]);
            $urls = Common_SecurityPlugin::getIssues($folder);
    ...
    private function addcode($req)
    {
        ...
        if (!Common_SecurityPlugin::validId($req["id_folder_codes"])) {
            throw new SoapFault("Server", $GLOBALS['ID_FILTER_INVALID']);
        }
        ...
        try {
            if (!Common_SecurityPlugin::validCode($req["code"])) {
    ...
    private function editcode($req)
    {
        ...
        if (!Common_SecurityPlugin::validId($req["id_code"])) {
            throw new SoapFault("Server", $GLOBALS['ID_FILTER_INVALID']);
        }
    ...
    private function getcodes($req)
    {
        ...
        try {
            $folder = $projectManager->getFolder($req["id_folder_codes"]);
            $codes = Common_SecurityPlugin::getIssues($folder);
    ...
    private function addserver($req)
    {
        ...
        if (!Common_SecurityPlugin::validId($req["id_folder_servers"])) {
            throw new SoapFault("Server", $GLOBALS['ID_FILTER_INVALID']);
        }

        if (!Common_SecurityPlugin::validName($req["hostname"])) {
            throw new SoapFault("Server", $GLOBALS['NAME_FILTER_INVALID']);
        }

        if (!Common_SecurityPlugin::validUse($req["use"])) {
        ...
        try {
            ...
            foreach ($ips as $ip) {
                if (!Common_SecurityPlugin::validIp($ip)) {
    ...
    private function getserverfromname($req)
    {
    ...
    private function editserver($req)
    {
        ...
        if (!Common_SecurityPlugin::validId($req["id_server"])) {
            throw new SoapFault("Server", $GLOBALS['ID_FILTER_INVALID']);
        }
    ...
    private function addissue($req)
    {
        ...
        if (!Common_SecurityPlugin::validId($req["id_folder_bugs"])) {
            throw new SoapFault("Server", $GLOBALS['ID_FILTER_INVALID']);
        }
    ...
    private function editissue($req)
    {
    ...
    private function addmember($req)
    {
        ...
        if (!Common_SecurityPlugin::validRights($req["access"])) {
            throw new SoapFault("Server", $GLOBALS['ACCESS_FILTER_INVALID']);
        }

        if (!Common_SecurityPlugin::validId($req["id_user"])) {
            throw new SoapFault("Server", $GLOBALS['ID_FILTER_INVALID']);
        }

        if (!Common_SecurityPlugin::validId($req["id_project"])) {
            throw new SoapFault("Server", $GLOBALS['ID_FILTER_INVALID']);
        }
    ...
    private function deletemember($req)
    {
    ...
    private function deleteproject($req)
    {
    ...
    private function editproject($req)
    {
    ...
    private function addproject($req)
    {

        $req = (array) $req;

            Common_SecurityPlugin::logp("tata1");
        if (!$this->authws()) {
            throw new SoapFault("Server", $GLOBALS['FAULT_AUTHENTICATION']);
        }

        if (!Common_SecurityPlugin::validName($req["name"], 40)) {
            throw new SoapFault("Server", $GLOBALS['NAME_FILTER_INVALID']);
        }

            Common_SecurityPlugin::logp("tata2");
        try {
            Common_SecurityPlugin::logp("tata2bis1");
            // check before if the project already exist and throw an exception if is
            $typeManager = new System_Api_TypeManager();
            $projectManager = new System_Api_ProjectManager();
            Common_SecurityPlugin::logp("tata2bis2");
            $type = $typeManager->getIssueType($GLOBALS['CONF_ID_TYPE_FOLDER_BUGS']); // Id bugs
            Common_SecurityPlugin::logp("tata2bis3");
            $projectId = $projectManager->addProject($req["name"]);
            Common_SecurityPlugin::logp("tata2bis4");
            $project = $projectManager->getProject($projectId);
            Common_SecurityPlugin::logp("tata2bis5");

            Common_SecurityPlugin::logp("tata3");
            if ($req["description"] != '') {
                $projectManager->addProjectDescription($project, $req["description"], System_Const::TextWithMarkup);
            }

            Common_SecurityPlugin::logp("tata4");
