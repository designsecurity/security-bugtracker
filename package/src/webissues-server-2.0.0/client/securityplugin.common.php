...
class CommonSecurityPlugin
{
    ...
    {
        return Common_SecurityPlugin::commonScan($req);
    }
    ...
    {
        $issueId = Common_SecurityPlugin::commonScan($req);
        ...
        try {
            ...
            $result = $clientsoap->__call('runOpenvas', array('run_openvas'=>$param));
    ...
    {
        if (Common_SecurityPlugin::validString($name) && strlen($name) > 1 && strlen($name) < $max) {
            return true;
        }
    ...
    {
        if (Common_SecurityPlugin::validInt($id)) {
            return true;
        }
